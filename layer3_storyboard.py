...
if __name__ == "__main__":
    # 加载 Layer 2 示例输出
    import sys
    input_path = "layer2_sample_output.json"
    if len(sys.argv) > 1:
    ...
    # 保存
    with open("layer3_sample_storyboard.json", "w", encoding="utf-8") as f:
        json.dump(script, f, ensure_ascii=False, indent=2)

    with open("layer3_sample_storyboard.txt", "w", encoding="utf-8") as f:
        f.write(text)

    print(f"分镜脚本已生成:")
    print(f"  JSON: layer3_sample_storyboard.json")
    print(f"  TEXT: layer3_sample_storyboard.txt")
    print()
    print(f"=== 结构概要 ===")
